...
fs.readFile(path, 'utf8', function(err, data) {
    ...
    while (k<bytes.length) {
        ...
        let bytesSubset = [...bytes].slice(0,k);
        let n = 1;
        while (n < 10000) {
            // console.log('n',n);
            ...
            for (let path of pathsTemp) {
                ...
                for (let move of moves) {
                    ...
                    if (newP[0] < 0 || newP[0] > xMax || newP[1] < 0 || newP[1] > yMax) {
                        ...
                        continue
                    } else if (bytesSubset.filter(b=>b[0]===newP[0] && b[1]===newP[1]).length > 0) {
        ...
        if (done) {
            ...
            maxOk= k;
            k = k + d;
            if (k >= bytes.length) {k=bytes.length-1}
            dir = 1;
        }
        else if (!done) {
            console.log('Failure!')
            if (k===maxOk+1) {break}
            if (d > 1) {d = Math.floor(d/2)}
            dir = -1;
            k = k - d;
        }
    }
    console.log('k',k,bytes[k-1])
